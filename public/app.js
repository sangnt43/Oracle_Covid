...
Start = (function() {
    ...
    function getSlideData(index) {
        return covid_world_timeline[index ? index : covid_world_timeline.length - 1];
    }
    ...
    window.mapData = JSON.parse(JSON.stringify(slideData.list));

    // remove items with 0 values for better performance
    for (var i = mapData.length - 1; i >= 0; i--) {
        if (mapData[i].confirmed == 0) mapData.splice(i, 1);

        mapData[i].confirmed = Number(mapData[i].confirmed);
        mapData[i].recovered = Number(mapData[i].recovered);
        mapData[i].deaths = Number(mapData[i].deaths);
        mapData[i].active = Number(mapData[i].active);
    }

    window.max = { confirmed: 0, recovered: 0, deaths: 0 };
    ...
    //#region Map polygon series (defines how country areas look and behave)
    ...
    function updateMapData(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var population =
                countries.find(x => x.iso2 == image.dataItem.dataContext.id).populations;
