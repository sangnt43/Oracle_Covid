...
DECLARE  ROWCOUNT INTEGER; XDAY NUMBER; XMONTH NUMBER; XYEAR NUMBER;TODAY DATE; XCONFIRMED NUMBER(38,0); XDEATHS NUMBER(38,0); XRECOVERED NUMBER(38,0);XACTIVE NUMBER(38,0);
BEGIN
    ...
    IF ROWCOUNT > 0 THEN
        UPDATE GLOBALS 
        SET CONFIRMED = CONFIRMED + :NEW.CONFIRMED, DEATHS = DEATHS + :NEW.DEATHS, 
            RECOVERED = RECOVERED + :NEW.RECOVERD, ACTIVE = ACTIVE + :NEW.ACTIVE
        WHERE :NEW.DATE_DAY = DATE_DAY AND :NEW.DATE_MONTH = DATE_MONTH AND :NEW.DATE_YEAR = DATE_YEAR;
    ELSE
